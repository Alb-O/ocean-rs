	...
	if is_interactive() {
		app.add_plugins(interactive_plugins(None))
			...
			.add_systems(
				...
				(
					setup_camera,
					setup_ocean.after(setup_camera),
					setup_ship,
					setup_environment,
				),
			)
		...
		let config =
			ScreenshotConfig::from_cli("05_skybox_reflect").with_presets(SKYBOX_PRESETS.to_vec());
...
fn setup_camera(mut commands: Commands) {
	// Position based on first preset (wide_horizon)
	let angle = 0.3_f32;
	let radius = 80.0_f32;
	let height = 25.0_f32;
	...
	commands.spawn((
		Camera3d::default(),
		Transform::from_xyz(x, height, z).looking_at(Vec3::new(0.0, 5.0, 0.0), Vec3::Y),
	));
}
...
fn setup_ship(mut commands: Commands, asset_server: Res<AssetServer>) {
	let ship_scene = asset_server.load(
		GltfAssetLabel::Scene(0)
			.from_asset("models/dutch_ship_medium_1k/dutch_ship_medium_1k.gltf"),
	);
